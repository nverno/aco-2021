...
struct Scanner {
    ...
    bool addBeacons(set<Point>& beacons, Point& loc) {
        calcRotations();
        for (auto& b : beacons) {
            for (auto& r : rots) {
                for (auto& origin : r) {
...
int main(int argc, char *argv[]) {
    ...
    Point loc(0,0,0);
    vector<Point> scanners = {loc};
    while (!q.empty()) {
        cur = q.front(); q.pop();
        if (!cur->addBeacons(beacons, loc))
            q.push(cur);
        else scanners.push_back(loc);
    }

    int res = 0;
    for (size_t i = 0; i < scanners.size(); ++i) {
        for (size_t j = i+1; j < scanners.size(); ++j) {
            res = max(res, scanners[i].manhattan(scanners[j]));
        }
    }
    cout << "max dist: " << res << endl;
    return 0;
}
