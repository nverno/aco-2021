...
struct node {
    int val, id;
    node* next, *sub;
    node(int v = -1): val(v), id(-1), next(nullptr), sub(nullptr) {}
};
...
int main(int argc, char *argv[]) {
    string sa, sb, div1 = "[[2]]", div2 = "[[6]]";
    vector<node*> packets;
    int di = 0, dj = 0;
    packets.push_back(parse_list(di, div1));
    packets.push_back(parse_list(dj, div2));
    packets[0]->id = 1;
    packets[1]->id = 2;
    ...
    sort(packets.begin(), packets.end(), [&](const auto& a, const auto& b) {
        return cmp(a, b) <= 0;
    });
    int res = 1;
